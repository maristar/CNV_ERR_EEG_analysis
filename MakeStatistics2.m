% Make statistical analysis 07-12-2011
clear all 
close all
...
textmeasuresall={'DTFdelta'}%,'pcor','DTFdelta','DTFtheta','DTFalpha','DTFbeta','DTFgamma'}
for qq=1:length(textmeasuresall)
    ...
    for kkm=3:ND
        ...
        if stemp~='.'
            cd(stemp)
            load resultscor
           meantemp=squeeze(mean(resultscor.resultsDTF.(textmeasure).meangamma,3)); %for %dtf
           %meantemp=squeeze(mean(resultscor.(textmeasure).result2,3));
