...
%% Filter
        ...
        %% Epoch 
        EEG=pop_epoch(EEG, { 'Go__' }, [-1 4.7], 'newname', [stemp '_sel_epoched']); % Here we change manually, Go__ eller NoGo
        EEG=pop_rmbase(EEG, [-1000 -50]);
        eeglab redraw
% now cut the useful interval of 1.4 until 4.75
        EEG = pop_select(EEG, 'time', [1.4 4.7]); %, EEG = pop_select(EEG, 'time', [3 4.75]); for the second component 'notrial', [28 34 49 51 56 57]); for 102
        eeglab redraw
        % Note 30.04.2015:
