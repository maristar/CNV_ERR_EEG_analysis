%%Analysis of CNV datasets. Maria L. S. 24.11.2011
...
% is excluded. Here it automatically selected 

clear all 
close all

cd('D:\RIKSHOSPITALET\CNV RIKS\RAW DATASETS\After_eye_art_removal')
files=dir('*.set');
...
%cd('F:\MEDISIN AT UIO NORWAY PC\DATA_DISK_MEDISIN\frontal lobe trauma\CNV RIKS\RAW DATASETS\Epoched\Go')
        EEG=pop_loadset(filenames{kkm}, 'D:\RIKSHOSPITALET\CNV RIKS\RAW DATASETS\After_eye_art_removal\');
        Fs=EEG.srate;
        eeglab redraw

        %% select channels
        numChannel=[7 6 106 62 72 70 75 83 23 24 27 3 124 123 36 35 40 110 104 109 92 97 91 52 51 59];
...
% now cut the useful interval of 1.4 until 4.75
        EEG = pop_select(EEG, 'time', [1.4 4.75]); %, 'notrial', [28 34 49 51 56 57]); for 102
        eeglab redraw
        ...
        'filepath',['D:\RIKSHOSPITALET\CNV RIKS\ANALYZED DATASETS\' stemp]); % no pop-up D:\RIKSHOSPITALET\CNV RIKS\ANALYZED DATASETS\'
        eeglab redraw
...
% save to results.. \ 
    ...
    resultscor.(textmeasure).pcor_couples=pcor_list;
...
% save to results.. \ 
    ...
    resultscor.(textmeasure).cor_couples=cor_list;
    resultscor.(textmeasure).br_areas=br_areas;
    clear chan_appearance_norm chan_strength_norm
...
%% DTF
    ...
    [DTFtheta]=DTF_maria_frontal(4, 8, p, Fs, data,thismoment, XYZ, N, name, stemp);
...
% delta..........
    ...
    [DTFdelta]=DTF_maria_frontal(0.1, 4, p, Fs, data,thismoment, XYZ, N, name, stemp);
...
% save to results.. go to RAW\ 
    ...
    [DTFalpha]=DTF_maria_frontal(9, 12, p, Fs, data,thismoment, XYZ, N, name, stemp);
...
% write to excel
    ...
    textmeasure='DTFbeta';
    [DTFbeta]=DTF_maria_frontal(13, 25, p, Fs, data,thismoment, XYZ, N, name, stemp);
...
% save to results.. go to RAW\ 
    ...
    [DTFgamma]=DTF_maria_frontal(25, 60, p, Fs, data,thismoment, XYZ, N, name, stemp);
    [DTFgamma_list]=majorlist(DTFgamma.meangamma, textmeasure, now, name, nchan, N)
    DTFgamma.couple_conn=DTFgamma_list.couple_conn;
    DTFgamma.couple_conn_values=DTFgamma_list.couple_conn_values;
    [chan_strength_norm, chan_appearance_norm]=strength_appearance(DTFgamma.couple_conn, DTFgamma.couple_conn_values,N);
    br_areas=brainar(chan_strength_norm, chan_appearance_norm);
    resultscor.(textmeasure).chan_strength_norm=chan_strength_norm;
    resultscor.(textmeasure).chan_appearance_norm=chan_appearance_norm;
    resultscor.(textmeasure).couples=DTFgamma_list;
    resultscor.(textmeasure).br_areas=br_areas;
    stempp=['MostCouples-' textmeasure '-stats']; % do not delete this!!!!
    xlswrite(stempp, {name}, 'Sheet1', 'A1:A1');
    titles={name};
...
%% save the results
    ...
    resultscor.resultsDTF.DTFgamma=DTFgamma;
