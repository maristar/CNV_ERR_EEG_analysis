...
for k=1:length(xyz)
text(xyz(k,2), xyz(k,1), B(k), 'FontSize',9, 'HorizontalAlignment', 'center','VerticalAlignment', 'bottom'); end
...
for k=1:length(xyz)
    for jj=1:length(xyz)
        if jj~=k
            ...
            if mean_result_temp>(max(connectivity_measure)*3/6), integerL=3;
