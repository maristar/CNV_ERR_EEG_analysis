...
% DOI='D:\RIKSHOSPITALET\CNV_RIKS\ANALYZED DATASETS_cube\NEW\Second_component_controls\' % found 2015.July.12
     ...
     clear Go1_left Go1_left_areas Go1_right Go1_right_areas NoGo1_left NoGo1_left_areas NoGo1_right NoGo1_right_areas
      ...
      for kkm=1:(ND-2)
          ...
          % Right Hemisphere
            ...
            thr=min(thr_go,thr_nogo);
            %% plot here with the same threshold
            cd('Pictures')
            cd(textmeasure)
            % Left hemisphere 
            side='Left'; Brain_areas={'FL', 'CL', 'PL'}; trigger='Go';
            figtemp=['Br-Areas-Lines-' name(1:5) '-' textmeasure '-' trigger '-' side];
            figure; plot3D_frontal(Go1_left, XYZ, numLeft,FL, CL, PL, thr); title(figtemp)
            saveas(gcf, figtemp, 'fig'); saveas(gcf, figtemp, 'jpeg')
    
            trigger='NoGo';
            figtemp=['Br-Areas-Lines-' name(1:5) '-' textmeasure '-' trigger '-' side];
            figure; plot3D_frontal(NoGo1_left, XYZ, numLeft,FL, CL, PL, thr); title(figtemp)
            saveas(gcf, figtemp, 'fig'); saveas(gcf, figtemp, 'jpeg')
          ...
          % Right Hemisphere
            ...
            FR=[1,8,9]; PR=[2,3,4]; CR=[5,6,7];
            figtemp=['Br-Areas-Lines-' name(1:5) '-' textmeasure '-' trigger '-' side];
            figure; plot3D_frontal(Go1_right, XYZ, numRight,FR, CR, PR, thr); title(figtemp)
            saveas(gcf, figtemp, 'fig'); saveas(gcf, figtemp, 'jpeg')
    
            trigger='NoGo';
            figtemp=['Br-Areas-Lines-' name(1:5) '-' textmeasure '-' trigger '-' side];
            figure; plot3D_frontal(NoGo1_right, XYZ, numRight, FR, CR, PR, thr); title(figtemp)
            saveas(gcf, figtemp, 'fig'); saveas(gcf, figtemp, 'jpeg')
            ...
            STATSFINAL(kkm).(textmeasure).thr_br_areas=thr;
            % here 24-2-2012 to continueee and finalize tomorrow!!
            
%             %INTeR - HEMISPHERIC on everyfile
%             
%             Left=STATSFINAL(14).(textmeasure).Go_left_areas_grandav;  %3 x 3
%             for kk=1:length(Left), for jj=1:length(Left); if kk==jj, Left(kk,jj)=0, else Left(kk,jj)=Left(kk,jj), end; end; end
%                     
%             suma1=sum(sum(Left));
%             arraysumLeft(kkm,:,:,:)=sum(Left);
%             Right=STATSFINAL(14).(textmeasure).Go_right_areas_grandav;
%             for kk=1:length(Right), for jj=1:length(Right); if kk==jj, Right(kk,jj)=0, else Right(kk,jj)=Right(kk,jj), end; end; end
%                     
%             suma2=sum(sum(Right));
%             arraysumRight(kkm,:,:,:)=sum(Right);
%             LRA=(suma1-suma2)/(suma1+suma2)
%             
%             STATSFINAL(14).(textmeasure).goLRA=LRA;
%             
%             LRA_go_F=arraysumLeft(kkm,1)-arraysumRight(kkm,1);
%             LRA_go_C=arraysumLeft(kkm,2)-arraysumRight(kkm,2);
%             LRA_go_P=arraysumLeft(kkm,3)-arraysumRight(kkm,2);
%             
%             
%             %NOGO
%             LeftN=STATSFINAL(14).(textmeasure).Go_left_areas_grandav;
%             for kk=1:length(LeftN), for jj=1:length(LeftN); if kk==jj, LeftN(kk,jj)=0, else LeftN(kk,jj)=LeftN(kk,jj), end; end; end
%                     
%             suma1=sum(sum(LeftN));
%             RightN=STATSFINAL(14).(textmeasure).Go_right_areas_grandav;
%             for kk=1:length(RightN), for jj=1:length(RightN); if kk==jj, RightN(kk,jj)=0, else RightN(kk,jj)=RightN(kk,jj), end; end; end
%             suma2=sum(sum(RightN));
%             LRA_nogo=(suma1-suma2)/(suma1+suma2)
%             STATSFINAL(14).(textmeasure).nogoLRA=LRA_nogo;
%             
      end

       %INTeR - HEMISPHERIC on grandav 
            last=size(STATSFINAL,2)
            Left=STATSFINAL(last).(textmeasure).Go_left_areas_grandav;  %3 x 3
            for kk=1:length(Left), for jj=1:length(Left); if kk==jj, Left(kk,jj)=0, else Left(kk,jj)=Left(kk,jj), end; end; end
                    
            suma1=sum(sum(Left));
            arraysumLeft(kkm,:)=sum(Left);
            Right=STATSFINAL(last).(textmeasure).Go_right_areas_grandav;
            for kk=1:length(Right), for jj=1:length(Right); if kk==jj, Right(kk,jj)=0, else Right(kk,jj)=Right(kk,jj), end; end; end
                    
            suma2=sum(sum(Right));
            arraysumRight(kkm,:,:,:)=sum(Right);
            
            LRA_go=(suma1-suma2)/(suma1+suma2);
            
        
            STATSFINAL(last).(textmeasure).goLRA=LRA_go;
            %STATSFINAL(14).(textmeasure).
            LeftN=STATSFINAL(last).(textmeasure).Go_left_areas_grandav;
            for kk=1:length(LeftN), for jj=1:length(LeftN); if kk==jj, LeftN(kk,jj)=0, else LeftN(kk,jj)=LeftN(kk,jj), end; end; end
                    
            suma1=sum(sum(LeftN));
            RightN=STATSFINAL(last).(textmeasure).Go_right_areas_grandav;
            for kk=1:length(RightN), for jj=1:length(RightN); if kk==jj, RightN(kk,jj)=0, else RightN(kk,jj)=RightN(kk,jj), end; end; end
            suma2=sum(sum(RightN));
            LRA_nogo=(suma1-suma2)/(suma1+suma2);
            STATSFINAL(last).(textmeasure).nogoLRA=LRA_nogo;
